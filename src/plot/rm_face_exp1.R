...
my_plot2 <-  ggplot() +
  ...
  geom_hline(aes(x = stimulus_types,
...
# plot: remove set size 3 to match spacing --------------------------------

data_across_subject3<-subset(data_across_subject2, spacing!="to_match")
...
data_by_subject4 <- data_preprocessed %>%
...
# data across subject
data_across_subject4 <- data_preprocessed %>%
...
# samplesize = 60 (each condition 20 repetition * 12 participant)
data_across_subject4 <- data_across_subject4 %>%
  mutate(deviation_socre_SEM = deviation_score_std / sqrt(240),
         rt_SEM = rt_std / sqrt(240))

my_plot4 <-  ggplot() +
  
  geom_point(data = data_across_subject4, aes(x = stimulus_types,
                                              y = deviation_score_mean,
                                              size = size_scale,
                                              color = size_scale),
  ...
  # each data point represents the average deviation of 1 participant
  geom_point(data = data_by_subject4, aes(x = stimulus_types,
                                             y = deviation_score_mean,
                                             size = size_scale,
                                          color = size_scale),
             alpha = 0.1,
  ...
  geom_errorbar(data = data_across_subject4, aes(x = stimulus_types,
                                                 y = deviation_score_mean,
                                                 ymin = deviation_score_mean - deviation_socre_SEM,
                                                 ymax = deviation_score_mean + deviation_socre_SEM,
                                                 group = size_scale),
